 ...
 * @author Superfluid
 */
contract SuperToken is SuperfluidToken, ISuperToken {
     ...
     */
    function _transferFrom(address spender, address holder, address recipient, uint256 amount)
        internal
        returns (bool)
    {
        if (holder == address(0)) {
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual override returns (bool) {
        _approve(
            msg.sender,
            spender,
            _allowances[msg.sender][spender].sub(subtractedValue, "SuperToken: decreased allowance below zero")
        );
        return true;
    }
    ...
    function granularity() external pure virtual override returns (uint256) {
        return 1;
    }
    ...
    function isOperatorFor(address operator, address tokenHolder) external view virtual override returns (bool) {
    ...
    function defaultOperators() external view virtual override returns (address[] memory) {
    ...
    function selfMint(address account, uint256 amount, bytes memory userData) external virtual override onlySelf {
        _mint(
            msg.sender,
            account,
            amount,
            userData.length != 0, /* invokeHook */
            userData.length != 0, /* requireReceptionAck */
            userData,
            new bytes(0)
        );
    }

    function selfBurn(address account, uint256 amount, bytes memory userData) external virtual override onlySelf {
        _burn(msg.sender, account, amount, userData.length != 0, /* invokeHook */ userData, new bytes(0));
    }

    function selfApproveFor(address account, address spender, uint256 amount) external virtual override onlySelf {
        _approve(account, spender, amount);
    }

    function selfTransferFrom(address holder, address spender, address recipient, uint256 amount)
        external
        virtual
        override
        onlySelf
    ...
    function transferAll(address recipient) external virtual override {
    ...
    function operationApprove(address account, address spender, uint256 amount) external virtual override onlyHost {
        _approve(account, spender, amount);
    }

    function operationIncreaseAllowance(address account, address spender, uint256 addedValue)
        external
        virtual
        override
        onlyHost
    ...
    function operationDecreaseAllowance(address account, address spender, uint256 subtractedValue)
        external
        virtual
        override
        onlyHost
    {
        _approve(
            account,
            spender,
            _allowances[account][spender].sub(subtractedValue, "SuperToken: decreased allowance below zero")
        );
    }

    function operationTransferFrom(address account, address spender, address recipient, uint256 amount)
        external
        virtual
        override
        onlyHost
    ...
    function operationSend(address spender, address recipient, uint256 amount, bytes memory userData)
        external
        virtual
        override
        onlyHost
    ...
    function operationUpgrade(address account, uint256 amount) external virtual override onlyHost {
        _upgrade(msg.sender, account, account, amount, "", "");
    }

    function operationDowngrade(address account, uint256 amount) external virtual override onlyHost {
